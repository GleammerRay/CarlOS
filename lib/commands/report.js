...
class Command {
  ...
  async execute(interaction) {
    ...
    if (msgs.code != null) {
      return await this.rest.createEphemeralInteractionResponse(interaction, { content: 'Failed to scan messages in this channel. :mag_right: :face_with_monocle:' });
    }
    ...
    if (lastMessage == null) {
      return await this.rest.createEphemeralInteractionResponse(interaction, { content: `Failed to find recent messages from <@${user}> in this channel, **try right clicking their message and reporting through the \`Apps\` section**. :mag_right: :face_with_monocle:` });
    }
    if (lastMessage.author.bot == true) {
      return await this.rest.createEphemeralInteractionResponse(interaction, { content: 'Reporting bots is not allowed. :robot:' });
    }
    ...
    if (reportMessage.id == null) {
      return await this.rest.createEphemeralInteractionResponse(interaction, { content: 'Failed to send report message. :no_entry_sign:' });
    }
