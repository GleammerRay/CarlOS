...
class Command {
  ...
  async execute(interaction) {
    ...
    if (topicThreadId == null) {
      const topicThread = await this.rest.startThread(settings.reportChannelId, `🚩 [${reason}]`, 60);
    ...
    if (reporterThreadId == null) {
      const reporterThread = await this.rest.startThread(settings.reportChannelId, `📣 Reporter - ${reporterName}`, 60);
    ...
    if (offenderThreadId == null) {
      const offenderThread = await this.rest.startThread(settings.reportChannelId, `🔴 Offender - ${offenderName}`, 60);
