...
class Command {
  ...
  async execute(interaction) {
    ...
    if (guildMember.code == 10007) {
      ...
      try {
        if (guildMember.user.avatar == null) {
          if (guildMember.user.discriminator == '0') {
            thumbnail = { url: `https://cdn.discordapp.com/embed/avatars/${(BigInt(userId) >> 22n) % 6n}.png` }; 
          } else {
            thumbnail = { url: `https://cdn.discordapp.com/embed/avatars/${Number(guildMember.user.discriminator) % 5}.png` }; 
          }
        } else {
          thumbnail = { url: `https://cdn.discordapp.com/avatars/${userId}/${guildMember.user.avatar}.png` };
        }
      } catch (e) {
        console.log(`W:Failed to decode user avatar:\n${e}`);
      }
    }
    ...
    return this.rest.createEphemeralInteractionResponse(interaction, { embeds: [ buildEmbed({ title: `User report summary.`, description: `${user.alive ? `Currently, <@${userId}> is a member of this server. :bust_in_silhouette:` : `Currently, <@${userId}> is not present in this server. :levitate:`}\n\n**:red_circle: Reports received against this user: ${user.reportsReceivedCount}**\n:mega: Reports sent against other members: ${user.reportsSentCount}\n:yellow_circle: Warnings received by this user: ${user.warningsReceivedCount}${user.warningsSentCount == 0 ? '' : `\n:crown: Warnings sent to other members: ${user.warningsSentCount}`}${ offenderThreadLink == reporterThreadLink ? '' : '\n'}${offenderThreadLink == null ? '' : `\n<#${offenderThreadLink}>`}${reporterThreadLink == null ? '' : `\n<#${reporterThreadLink}>`}`, thumbnail: thumbnail }) ], 'allowed_mentions': { 'parse': [] } });
  }
